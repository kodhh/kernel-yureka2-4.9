...
extern ssize_t fuse_passthrough_vfs_iter_read(struct file *file, struct iov_iter *iter, loff_t *ppos,
		int flags);

extern ssize_t fuse_passthrough_vfs_iter_write(struct file *file, struct iov_iter *iter, loff_t *ppos,
		int flags);
				   ...
				   struct iov_iter *iter)
{
	...
	if (is_sync_kiocb(iocb_fuse)) {
		ret = fuse_passthrough_vfs_iter_read(passthrough_filp, iter, &iocb_fuse->ki_pos, (iocb_fuse->ki_flags & PASSTHROUGH_IOCB_MASK));
	} else {
		struct fuse_aio_req *aio_req;
				    ...
				    struct iov_iter *iter)
{
	...
	if (is_sync_kiocb(iocb_fuse)) {
		...
		ret = fuse_passthrough_vfs_iter_write(passthrough_filp, iter, &iocb_fuse->ki_pos, (iocb_fuse->ki_flags & PASSTHROUGH_IOCB_MASK));
